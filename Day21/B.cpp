...
ll get_length_recursive(string str, int depth, vector<map<string, ll>> & dp){
  if(depth == 0)
    return SZ(str);
  ...
  for(auto & c : str){
    len += get_length_recursive(directional_moveset[directional_ID(last_char)][directional_ID(c)], depth - 1, dp);
    last_char = c;
  }
...
ll complexity(string & code, int depth){
  ...
  for(auto & c : code){
    len += get_length_recursive(numerical_moveset[numerical_ID(last_char)][numerical_ID(c)], depth, dp);
    last_char = c;
  }
